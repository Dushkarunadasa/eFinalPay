...
{
    public class RecAuthorizeController : Controller
    {
        ...
        public RecAuthorizeController(IMainServices Iss, IPayDetails IPayIss, ILoginDetails ILog, IRecoveryServices IRec, IHttpContextAccessor accessor)
        {
            _Iss = Iss;
            _IPayIss = IPayIss;
            _ILog = ILog;
        ...
        public async Task<IActionResult> ForwardRecovery(int id)
        {
            ...
            else
            {
               ...
               await _IRec.updateLog(new SubFinalPayLogDetail { TransId = id, UnitId = _ILog.getUnitID(), UserId = _ILog.getUserName().ToString(), LogDetails = _ILog.GetUserRoll().ToString() + " forwarded tansaction to SSO(SM)", Logtime = DateTime.Now, Ip = IP });
        ...
        public async Task<IActionResult> RecADelete(int id, string ItemCode)
        {
